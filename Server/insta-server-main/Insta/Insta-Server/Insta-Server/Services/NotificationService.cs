        ...
        private static bool CommonPostNotificate(NotificationType type, string actor, int postId, IHubContext<NotificationHub> context)
        {
            try
            {
                ...
                if (type == NotificationType.CommentNotification)
                {
                    ...
                    .SetType(type, postId+"")
                    .SetContent($"{username} " + (numberPeopleCommented.Count == 0 ? "": $"and {numberPeopleCommented.Count} other(s) ") +"commented in ? post.")
                    .SetUserId(actor)
                    .Build();
                } else
                {
                    ...
                    notify = new NotificationBuilder()
                    .SetType(type, postId +"")
                    .SetContent($"{username} " + (numberPeopleCommented.Count == 0 ? "": $"and {numberPeopleCommented.Count} other(s) ") +"reacted ? post.")
                    .SetUserId(actor)
                    .Build();
                }
